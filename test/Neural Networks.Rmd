
```{r}
burundi_crs <- readRDS("../data/bdi_result_crs.rds")
```

# Drafting Visualization
```{r}
library(visNetwork)
library(tidygraph)
library(dplyr)
library(DT)
```

```{r}
# Subsetting to Burundi's 2005 data
burundi_crs_2005 <- burundi_crs[["edge_lists"]][["2005"]]
```

# Checking if contract id is unique
```{r}
duplicates <- burundi_crs_2005 %>%
  count(id) %>%
  filter(n > 1)
duplicates
```
```{r}
burundi_duplicates <- left_join(duplicates, burundi_crs_2005, by = "id")
burundi_duplicates
```
There are duplicates which would need to be further investigated.

- can a sender by a receiver and vice versa?
```{r}
library(tibble)

test_data <- tibble(
  sender_in_receiver = burundi_crs_2005$sender %in% burundi_crs_2005$receiver
)
subset(burundi_crs_2005, sender %in% receiver)
```
its possible. I also did a visually inspection on if the organization type matches based on if the organization is sender and receiver and it does match so no issues is suspected in regards to that.

# Creating the nodes dataframe

for the nodes dataframe i need:
- id = Sender and Receiver uniquely appended
- label = id
- group = organization types uniquely appended

```{r}
nodes <- burundi_crs_2005 %>%
  mutate(id = sender, group = sender_orgtype) %>%
  select(id, group) %>%
  bind_rows(
    burundi_crs_2005 %>%
      mutate(id = receiver, group = receiver_orgtype) %>%
      select(id, group)
    ) %>%
  distinct(id, .keep_all = TRUE) %>%
  mutate(label = id)
```


# Creating the edges dataframe

For the edges, I will need:
- from = sender
- to = receiver
- value = (for now this will be the number of contracts. In the shiny app this will be customizeable between cost too)
- title = This can be a custom message. something saying "this is a contract from <sender> to <receiver>. the contract id is <id>. the number of contract between them is <n_contracts>. the sector is <sector>.

```{r}
edges <- burundi_crs_2005 %>%
  mutate(from = sender, to = receiver) %>%
  select(from, to)
```

```{r}
# this will be dependent on the user selection (pick one option)

#option 1
edges$value <- burundi_crs_2005$cost

#option 2
n_contract_sum <- burundi_crs_2005 %>%
  group_by(sender, receiver) %>%
  summarise(value = n_distinct(id), .groups = "drop") %>%
  mutate(from = sender, to = receiver) %>%
  select(from, to, value)

edges <- left_join(edges, n_contract_sum, by = c("from", "to")) %>%
  distinct(from, to, value, .keep_all = TRUE)
```


# Network Visualization
```{r}
visNetwork(nodes, edges)
```


```{r}
# Creating visualization
visNetwork(nodes, edges) %>%
  visNodes(shape = "dot", scaling = list(min = 10, max = 30)) %>%
  visEdges(smooth = TRUE) %>%
  visOptions(highlightNearest = TRUE, nodesIdSelection = TRUE) %>%
  visGroups(groupname = "Government", color = "lightblue") %>%
  visGroups(groupname = "NGO", color = "lightgreen") %>%
  visGroups(groupname = "Private", color = "orange") %>%
  visLegend() %>%
  visLayout(randomSeed = 123)
```

```{r}
visNetwork(nodes, edges)
```

```{r}
nodes_example <- data.frame(id = 1:10, 
                    
  # add labels on nodes
  label = paste("label Node", 1:10),
  
  # add groups on nodes 
  group = c("GrA", "GrB"),
  
  # size adding value
  value = c(1,2,3,4,5,6,7,8,9,20),          
  
  # control shape of nodes
  shape = c("diamond", "diamond", "diamond", "diamond", "diamond", "diamond",
  "diamond", "diamond", "diamond", "diamond"),
  
  # tooltip (html or character), when the mouse is above
  title = paste0("<p><b>", 1:10,"</b><br>Node !</p>"),
  
  # color
  color = c("darkred", "grey", "orange", "darkblue", "purple"),
  
  # shadow
  shadow = c(FALSE, TRUE, FALSE, TRUE, TRUE))             

# head(nodes)
# id  label group value    shape                     title    color shadow
#  1 Node 1   GrA     1   square <p><b>1</b><br>Node !</p>  darkred  FALSE
#  2 Node 2   GrB     2 triangle <p><b>2</b><br>Node !</p>     grey   TRUE

edges_example <- data.frame(from = c(1,2,5,7,8,10), to = c(9,3,1,6,4,7), value = c(0,2,3,4,5,26), title = c("title/ tool tip"))
```


```{r}
set.seed(123)
visNetwork(nodes_example, edges_example, height = "500px", width = "100%") %>%
  visEdges(arrows = "to")
```


## Questions
- there are duplicate contract IDs, ask about them. (I only check Burundi 2005)
- There are negative contracts




# Testing the Plotly Library
```{r}
library(plotly)
library(igraph)
library(igraphdata)
```
```{r}
data(karate, package ="igraphdata")
G <- upgrade_graph(karate)
L <- layout.circle(G)
```
```{r}
vs <- V(G)
es <- as.data.frame(get.edgelist(G))

Nv <- length(vs)
Ne <- length(es[1]$V1)
```

```{r}
library(plotly)

Xn <- L[,1]
Yn <- L[,2]

network <- plot_ly(x = ~Xn, y = ~Yn, mode = "markers", text = vs$label, hoverinfo = "text")
```

```{r}
edge_shapes <- list()
for(i in 1:Ne) {
  v0 <- es[i,]$V1
  v1 <- es[i,]$V2

  edge_shape = list(
    type = "line",
    line = list(color = "#030303", width = 0.3),
    x0 = Xn[v0],
    y0 = Yn[v0],
    x1 = Xn[v1],
    y1 = Yn[v1]
  )

  edge_shapes[[i]] <- edge_shape
}

```

```{r}
axis <- list(title = "", showgrid = FALSE, showticklabels = FALSE, zeroline = FALSE)

fig <- layout(
  network,
  title = 'Karate Network',
  shapes = edge_shapes,
  xaxis = axis,
  yaxis = axis
)

fig

```

# Testing Sigmajs

```{r}
library(sigmajs)

set.seed(123)

# generate data
nodes <- sg_make_nodes(25) # 20 nodes
nodes <- nodes %>%
  mutate(label = id)

edges <- sg_make_edges(nodes, 50) # 50 edges

edges <- edges %>%
  mutate(size =1:nrow(edges),
         label = id,
         type = "arrow")

edges <- edges %>%
  mutate(size = scales::rescale(size, to = c(10,50)))

sigmajs() %>% # initialise
  sg_nodes(nodes, id, label, size) %>% # add nodes
  sg_edges(edges, id, source, target, size, label, type) %>% # add edges
  sg_layout() %>%  # layout
  sg_cluster() %>% # cluster
  sg_drag_nodes() %>% # allows user to drag nodes
  sg_neighbours() # show node neighbours on node click
```
### chatgpt testing
```{r}
library(sigmajs)
library(dplyr)
library(htmlwidgets)

set.seed(123)

# Generate node and edge data
nodes <- sg_make_nodes(25) %>%
  mutate(label = id)

edges <- sg_make_edges(nodes, 50) %>%
  mutate(
    size = scales::rescale(1:nrow(.), to = c(10, 50)),
    label = id,
    type = "arrow"
  )

# Build sigmajs visualization with JavaScript arrow override
s <- sigmajs() %>% 
  sg_nodes(nodes, id, label, size) %>% 
  sg_edges(edges, id, source, target, size, label, type) %>% 
  sg_layout() %>% 
  sg_cluster() %>% 
  sg_drag_nodes() %>% 
  sg_neighbours()

# Inject JavaScript to increase arrowhead size
onRender(s, "
  function(el, x) {
    if (el && el.sigma) {
      el.sigma.settings('minArrowSize', 50);
      el.sigma.settings('defaultEdgeType', 'arrow');
      el.sigma.refresh();
    }
  }
")

```



nodes data need:
- id
- label
- size
- color

```{r}
burundi_crs <- readRDS("../data/bdi_result_crs.rds")
burundi_crs_2005 <- burundi_crs[["edge_lists"]][["2005"]]

sg_nodes <- burundi_crs_2005 %>%
  mutate(id = sender, group = sender_orgtype) %>%
  select(id, group) %>%
  bind_rows(
    burundi_crs_2005 %>%
      mutate(id = receiver, group = receiver_orgtype) %>%
      select(id, group)
    ) %>%
  distinct(id, .keep_all = TRUE) %>%
  mutate(label = id)

sg_nodes <- sg_nodes %>%
  mutate(size = 3,
         color = "#000000") %>%
  select(id, label, size, color, everything())
```

in the edges data i need:
- id
- source
- target

```{r}
test_edges <- burundi_crs_2005 %>%
  mutate(source = sender, target = receiver) %>%
  select(id, source, target)

test_edges <- test_edges%>%
  mutate(id = 1:length(test_edges$id),
         size = 1:nrow(test_edges))
```

```{r}
# this will be dependent on the user selection (pick one option)

#option 1
edges$value <- burundi_crs_2005$cost

#option 2
n_contract_sum <- burundi_crs_2005 %>%
  group_by(sender, receiver) %>%
  summarise(value = n_distinct(id), .groups = "drop") %>%
  mutate(from = sender, to = receiver) %>%
  select(from, to, value)

edges <- left_join(edges, n_contract_sum, by = c("from", "to")) %>%
  distinct(from, to, value, .keep_all = TRUE)
```


## sigma viz
```{r}
rm(sg_nodes)

sigmajs() %>%
  sg_nodes(test_nodes, id, label, size, color) %>%
  sg_edges(test_edges, id, source, target, size) %>%
  sg_layout() %>%
  sg_cluster() %>%
  sg_drag_nodes() %>%
  sg_neighbors() %>%
  sg_settings(labelThreshold = 15) 
```

# NetworkD3
```{r}
library(networkD3)
```

